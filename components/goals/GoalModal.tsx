...
export function GoalModal({ isOpen, onClose, onSave, initialData }: GoalModalProps) {
    ...
    useEffect(() => {
        if (isOpen && initialData) {
            ...
            const iDate = initialData.targetDate || initialData.target_date;
            setTargetDate(iDate ? new Date(iDate).toISOString().split('T')[0] : "");
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onSave({
            title,
            description,
            targetDate: targetDate ? `${targetDate}T12:00:00` : null, // Handle empty date safely
            imageUrl,
            status,
            tasks
        });
